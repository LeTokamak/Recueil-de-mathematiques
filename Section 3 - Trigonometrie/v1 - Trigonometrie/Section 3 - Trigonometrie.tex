...
\newpage
	...
	\section{Cercle trigonométrique}
		...
		\subsection{Lien avec les fonctions trigonométriques}
			...
			\begin{itemize}
				\item [•] $\text{Cathète Adjacente} = \cos(\theta) \times \text{Hypoténuse}$ $\Longrightarrow$ Longueur de $[OX_M]$ = $\cos(\theta) \times 1$
				\item [•] $\text{Cathète Opposée  } = \sin(\theta) \times \text{Hypoténuse}$ $\Longrightarrow$ Longueur de $[X_MM]$ = $\sin(\theta) \times 1$
			\end{itemize}
			...
			en projetant ce point sur l'axe des abscisses et sur celui des ordonnées.
			...
			---TO DO--- : Ajouter une image pour illustrer ce point.

			De plus, on peut aussi retrouver graphiquement la tangente, 
			pour un angle $\theta$, c'est la longueur du segment perpendiculaire à la droite (OA) tangente (géométrique) au cercle trigonométrique qui intercepte l'axe des abscisses.
