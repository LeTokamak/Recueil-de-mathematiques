...
\newpage
	...
	\section{Formulaire trigonométriques}
		...
		\subsection*{Définition}
			...
			\begin{tabular}{ccccc}
				...
				&&&&\\
				\Large{$\cos(x) = \frac{e^{ix} + e^{-ix}}{2}$} & $\quad \text{et} \quad$ & \Large{$\sin(x) = \frac{e^{ix} - e^{-ix}}{2i}$} & $\quad \text{et} \quad$ & \Large{$\tan(x) = \frac{\sin(x)}{\cos(x)}$}\\
		...
		\subsection*{Formules d'addition}
			...
			\begin{tabular}{lll|c}
				\large{$\cos(a + b)$} & \large{$=$} & $\cos(a) \cos(b) - \sin(a) \sin(b)$ & En développant $e^{i(a+b)} = e^{ia} e^{ib}$, puis en \\
				\large{$\sin(a + b)$} & \large{$=$} & $\cos(a) \sin(b) + \sin(a) \cos(b)$ & identifiant partie réelle et partie imaginaire.\\
				\hline
