	...
	car cette unité est plus facilement manipulable.
		...
		\subsection{Approximation linéaire}
			...
			\subsubsection{Interpolation linéaire}
				...
				En utilisant cette approximation, nous supposons que la fonction $f$ évolue linéairement entre deux points de la table.  
				
				Avec cette hypothèse et les points $A \left(a~; f(a)\right)$ et $B \left(b~; f(b)\right)$, 
				qui encadrent $x$ dans la table (avec $a < x < b$), 
				nous pouvons obtenir une valeur approchée de $f(x)$,
				en utilisant la formule suivante :

					$$f(x) \approx f(a) + \frac{f(b) - f(a)}{b - a} \times (x - a)$$
